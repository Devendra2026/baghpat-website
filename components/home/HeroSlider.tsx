...
// import { Activity, ArrowRight, Building2, CheckCircle, ChevronLeft, ChevronRight, Sparkles, Trees } from 'lucide-react';
...
//       description:
//         'Dedicated to keeping Baghpat clean, green, and open-defecation-free with efficient daily waste management, compost production, and plastic-free campaigns.',
...
//       description:
//         'Bringing municipal services directly to your screen. Apply for certificates, renew trade licenses, pay taxes, and monitor status updates in real-time.',
...
//       description:
//         'Constructing modern concrete roads, scientific drainage lines, public health parks, and replacing older street lights with energy-efficient Smart LEDs.',
...
//       description:
//         'Developing oxygen parks, implementing rain-water harvesting mandates, and expanding green covers along highways to enhance environmental sustainability.',
...
//   return (
//     <section className="relative w-full h-162.5 overflow-hidden bg-slate-950" aria-label="Municipal Projects Showcase">
//       {/* Slides Container */}
...
//             className={`absolute inset-0 w-full h-full bg-linear-to-tr ${slides[current].bgGradient} flex items-center justify-center`}
//           >
...
//               </svg>
//               <svg
//                 className="absolute -right-20 -top-20 w-125 h-125 text-white"
//                 viewBox="0 0 100 100"
//                 fill="currentColor"
//               >
...
interface Slide {
  ...
  theme: string;
  icon: React.ReactNode;
  imageUrl: string; // Changed from bgGradient to imageUrl
  primaryCta: string;
  primaryHref: string;
  secondaryCta: string;
}
...
export default function HeroSlider() {
  ...
  const slides: Slide[] = [
    ...
    //   imageUrl: 'https://cdn.s3waas.gov.in/s30336dcbab05b9d5ad24f4333c7658a0e/uploads/2026/04/17775345344929.png',
    //   primaryCta: 'File Complaint / Register Feedback',
    //   primaryHref: '#citizen-services',
    //   secondaryCta: 'View Ward Statistics',
    // },
    {
      ...
      theme: 'blue',
      icon: <Activity className="w-12 h-12 text-sky-400" />,
      imageUrl: 'https://cdn.s3waas.gov.in/s30336dcbab05b9d5ad24f4333c7658a0e/uploads/2026/04/17775345298586.png',
      primaryCta: 'Access Digital Services',
      primaryHref: '#citizen-services',
      secondaryCta: 'Download Mobile App',
    },
    {
      id: 3,
      titleHi: 'हरित एवं स्वस्थ बागपत पर्यावरण पहल',
      titleEn: 'Green & Healthy Baghpat',
      description:
        'Developing oxygen parks, implementing rain-water harvesting mandates, and expanding green covers along highways to enhance environmental sustainability.',
      badge: 'Eco-Friendly Campaign',
      theme: 'teal',
      icon: <Trees className="w-12 h-12 text-emerald-400" />,
      // Beautiful Indian public eco-park layout with rich greenery
    ...
    {
      id: 4,
      titleHi: 'नगर का सर्वांगीण बुनियादी ढांचा एवं जन स्वास्थ्य विकास',
      titleEn: 'Urban Infrastructure & Public Health',
      description:
        'Constructing modern concrete roads, scientific drainage lines, public health parks, and replacing older street lights with energy-efficient Smart LEDs.',
      badge: 'Development Projects',
      theme: 'green',
      icon: <Building2 className="w-12 h-12 text-teal-400" />,
      // Modern clean city road with integrated solar-LED pathways and green trees
      imageUrl: 'https://images.unsplash.com/photo-1541336032412-2048a678540d?auto=format&fit=crop&w=1920&q=80',
      primaryCta: 'Browse active tenders',
      primaryHref: '#news-notices',
      secondaryCta: 'Completed Project reports',
    },
  ...
  return (
    <section className="relative w-full h-[650px] overflow-hidden bg-slate-950" aria-label="Municipal Projects Showcase">
          ...
          >
            ...
            <div className="max-w-6xl mx-auto px-4 md:px-8 w-full relative z-10 text-white flex flex-col items-start gap-4">
              <m.div
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.2 }}
                className="flex items-center gap-2 bg-slate-900/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 text-xs font-bold uppercase tracking-widest text-amber-400"
              >
                <div className="scale-75 origin-center">{slides[current].icon}</div>
                <span>{slides[current].badge}</span>
              </m.div>
              ...
              <m.h2
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.3 }}
                className="text-2xl md:text-4xl font-extrabold tracking-tight font-serif text-amber-100 drop-shadow-md"
              >
                {slides[current].titleHi}
              </m.h2>
              ...
              <m.h3
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.4 }}
                className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tight uppercase leading-none text-white drop-shadow-lg"
              >
                {slides[current].titleEn}
              </m.h3>
              ...
              <m.p
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.5 }}
                className="max-w-2xl text-sm md:text-base text-slate-200 font-medium leading-relaxed mt-2 drop-shadow-sm"
              >
                {slides[current].description}
              </m.p>
              ...
              >
                <a
                  href={slides[current].primaryHref}
                  className="flex items-center gap-2 px-6 py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-lg text-xs md:text-sm font-bold uppercase tracking-wider transition-all shadow-lg hover:scale-105"
                >
                  <span>{slides[current].primaryCta}</span>
                  <ArrowRight className="w-4 h-4" />
                </a>
                <a
                  href="#about-us"
                  className="flex items-center gap-1.5 px-6 py-3 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-lg text-xs md:text-sm font-bold uppercase tracking-wider border border-white/20 transition-all hover:scale-105 backdrop-blur-xs"
                >
      ...
      <button
        onClick={handlePrev}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-slate-900/60 hover:bg-amber-500 text-white hover:text-slate-950 p-3 rounded-full hover:scale-110 border border-white/10 transition-all cursor-pointer backdrop-blur-xs"
        aria-label="Previous slide"
      >
        <ChevronLeft className="w-6 h-6" />
      </button>
      <button
        onClick={handleNext}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-slate-900/60 hover:bg-amber-500 text-white hover:text-slate-950 p-3 rounded-full hover:scale-110 border border-white/10 transition-all cursor-pointer backdrop-blur-xs"
        aria-label="Next slide"
      >
        <ChevronRight className="w-6 h-6" />
      </button>
      ...
      <div className="absolute bottom-6 left-0 right-0 z-20 flex justify-center items-center gap-2">
        {slides.map((slide, idx) => (
          <button
            key={slide.id}
            onClick={() => setCurrent(idx)}
            className={`h-3 rounded-full border border-white/10 transition-all duration-300 cursor-pointer ${
              idx === current ? 'bg-amber-400 w-8 scale-105' : 'bg-white/40 w-3 hover:bg-white/70'
            }`}
            aria-label={`Go to slide ${idx + 1}`}
          />
        ))}
